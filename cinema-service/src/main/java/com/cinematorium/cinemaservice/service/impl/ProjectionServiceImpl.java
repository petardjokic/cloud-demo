...
public class ProjectionServiceImpl implements ProjectionService {
	...
	public List<ProjectionEager> getByCinemaId(Long cinemaId) {
		...
		ResponseEntity<List<Projection>> response = restTemplate.exchange(
				"http://projection-service/api/projections/" + cinemaId, HttpMethod.GET, null,
		...
		projections.stream().forEach(projection -> {
			Movie movie = restTemplate.getForObject("http://movie-info-service/api/movies/" + projection.getMovieId(), Movie.class);
